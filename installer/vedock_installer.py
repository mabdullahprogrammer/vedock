...
const locationInput=()=>document.getElementById('installLocation');async function showInstalled(){install.disabled=false;install.textContent='Open Vedock';message.textContent='Vedock is current — nothing will be downloaded';message.className='success';percent.textContent='100%';bar.style.width='100%';install.onclick=()=>window.pywebview.api.launch()}function showReady(state){install.disabled=false;install.onclick=beginInstall;install.textContent=state.update_available?'Update Vedock':'Install Vedock';message.textContent=state.update_available?'A newer connected client is ready':'Ready to install';message.className='';percent.textContent='0%';bar.style.width='0%'}async function init(){let state=await window.pywebview.api.state();locationInput().value=state.location;state.installed?showInstalled():showReady(state)}async function chooseFolder(){let value=await window.pywebview.api.choose_folder();if(value){locationInput().value=value;let state=await window.pywebview.api.check_installation(value);state.installed?showInstalled():showReady(state)}}async function beginInstall(){let state=await window.pywebview.api.check_installation(locationInput().value);if(state.installed){showInstalled();return}install.disabled=true;install.textContent=state.update_available?'Updating…':'Installing…';await window.pywebview.api.install(locationInput().value,shortcut.checked)}window.installProgress=(text,value,level)=>{message.textContent=text;message.className=level;percent.textContent=value+'%';bar.style.width=value+'%'};window.installComplete=()=>showInstalled();window.installFailed=()=>{install.disabled=false;install.textContent='Try again'};window.addEventListener('pywebviewready',init);
</script></body></html>"""

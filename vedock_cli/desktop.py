...
class DesktopBridge:
    ...
    def choose_folder(self) -> str:
        ...
        selected = webview.windows[0].create_file_dialog(webview.FOLDER_DIALOG, allow_multiple=False)
        return str(selected[0]) if selected else ""
...
const logo="__LOGO__";let state={boot:null,jobs:[],models:[],datasets:[],resources:[],page:"tasks"};const api=(name,...args)=>window.pywebview.api[name](...args);const esc=v=>String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));const gb=n=>n?`${(n/1073741824).toFixed(1)} GiB`:"Unknown";
